...
- Portee d'implementation future: capture sans UI, chemin temps reel, file SPSC, stockage SQLite et preuve coffre via un `VaultHarness` strictement test-only
...
- Contrats relus: product-architecture SHA `3f3a6d2`; les corrections H-01/M-01 de ce plan n'en promeuvent aucune implementation
...
| Capture et backend coffre du harnais par OS | `platform-lead` | API, permission et environnement nommes; harnais jetable hors binaire produit; aucun comportement macOS/Linux deduit de Windows |
...
| Manifests, lockfiles et sources de dependances | owner designe + `security-reviewer` | gate F-03 versionne termine avant toute mutation, y compris pour le harnais |

Stopper et arbitrer avant implementation si un port normatif est encore ambigu, si deux leads proposent des formats incompatibles, si une dependance native ou un `unsafe` est necessaire sans revue, si un test exige des donnees reelles, ou si le changement impose un manifeste/lockfile sans ownership explicite du manager et sans gate F-03. Avant toute ligne de code C4, choisir explicitement le `VaultHarness` jetable de ce plan ou faire accepter un ADR securite et un contrat produit versionne; l'absence de choix bloque le code.
...
dependency-inventory.json  # campagne F-01/F-02/F-03 uniquement
```

`<run-id>` suit `YYYYMMDD-HHMMSS-<commit-court>-<scenario>`. `stdout.txt` et `stderr.txt` sont expurges avant archivage. Les fichiers ne contiennent ni audio, ni transcription, ni secret factice en clair, ni requete SQL libre, ni chemin personnel. Un canari de retention non-C4 peut etre identifie par son SHA-256. Le `SecretMaterial` C4 du `VaultHarness` n'est jamais exporte, pas meme sous forme de hash ou d'empreinte; seuls codes, durees et compteurs C1 sortent de la frontiere.
...
| `vault-handle` | `HarnessOperationId`, namespace et handle opaque C1; aucun C4 dans la fixture | `SecretMaterial` genere et possede uniquement dans `VaultHarness`; handle seul en SQLite |

Aucune fixture ne contient voix, dictee, adresse, credential reel, C4 ou chemin utilisateur. Le namespace du `VaultHarness` est unique au `run-id`; son nettoyage est idempotent et verifie meme apres echec.
...
### 4.5 `VaultHarness` C4 strictement test-only

Rust core possede la preuve de separation et le lifecycle du harnais; `platform-lead` possede l'appel au backend natif de test. `VaultHarness`, ses types, son namespace et ses dependances sont jetables, non promouvables et absents du binaire produit. Ils ne constituent ni un sixieme port produit, ni un choix de crate. Tout besoin C4 futur dans le produit exige d'abord un ADR securite et un contrat versionne approuves, puis le gate F-03 avant la moindre mutation de manifeste/lockfile.

| Plateforme | Coffre a exercer si environnement disponible | Preuve minimale du harnais jetable |
|---|---|---|
| Windows | Credential Manager sur `HW-WIN` actuel | create/read medie/replace/delete/restart, refus/verrou/crash/logout et cleanup du namespace |
| macOS | Keychain sur Apple Silicon seulement si machine disponible | memes operations, prompts et etats documentes sans sortie C4 |
...
Le `VaultHarness` reserve un namespace de campagne et possede `SecretMaterial` depuis sa generation interne jusqu'a sa suppression. L'appelant ne possede qu'un `SecretHandle` opaque C1 et un `HarnessOperationId`. Une lecture/comparaison est `UseMediated`: elle s'execute dans la frontiere coffre et ne retourne qu'un statut C1. La valeur C4 n'est jamais retournee, clonee, loggee, serialisee ou exportee, et n'apparait jamais dans arguments CLI, variables d'environnement, fixtures, fichiers, IPC/WebView, stdout/stderr, artefacts, SettingsStore ou SQLite. Le scan de fuite couvre la memoire serialisable du harnais, DB/WAL/SHM, fixtures, logs, stdout/stderr, artefacts et IPC; il s'execute dans la frontiere du harnais et n'exporte que le nombre de correspondances. Aucun hash ou fingerprint du C4 n'est conserve.

L'ordre obligatoire interne est `ReserveNamespace -> ProvisionSynthetic -> UseMediated -> Delete -> VerifyEmpty`. Chaque mutation porte un `HarnessOperationId`; un doublon confirme rend le meme etat sans creer ou remplacer une valeur. Les seuls etats de mutation du harnais sont `Applied`, `NotApplied` et `MutationUnknown`. `MutationUnknown` est l'unique outcome inconnu du harnais et reste fail-closed: il interdit tout retry ou reprovisionnement automatique sous le meme identifiant et met le namespace en quarantaine jusqu'a un probe/delete explicite. Il est distinct de la recoverability produit `OutcomeUnknown` reservee a Delivery. Ce type est test-only, ne complete pas `PortError` et ne traverse jamais l'IPC produit.

#### Ordre SQLite/coffre et compensation

Une transaction distribuee n'est jamais presumee. Pour une operation de harnais qui doit associer un handle a SQLite, le worker commence toujours par committer une intention C1 contenant seulement namespace, `HarnessOperationId`, operation attendue, handles opaques concernes et phase; aucun secret, hash C4 ou donnee libre. Il suit ensuite l'ordre test-only par operation:

| Operation | Ordre apres journal d'intention | Compensation/restart |
|---|---|---|
| create | `ProvisionSynthetic(newHandle)` puis CAS SQLite pour publier le handle, puis clore l'intention | si le CAS echoue apres `Applied`, `Delete(newHandle)` idempotent; issue inconnue => quarantaine |
| read | charger le handle opaque puis `UseMediated`; aucune valeur C4 ni mutation SQLite | statut C1 seulement; handle absent/refus/verrou explicite, aucun fallback |
| replace | provisionner un nouveau handle, CAS SQLite ancien->nouveau, puis delete idempotent de l'ancien | CAS echoue => delete du nouveau; delete ancien inconnu => quarantaine, jamais reprovision automatique |
| delete | `Delete(handle)` puis, seulement sur `Applied`, retirer le handle et clore l'intention | `NotApplied` conserve l'association et rend l'echec; commit SQLite perdu apres `Applied` => restart probe/VerifyEmpty puis suppression du handle stale, jamais recreation |

Sur `MutationUnknown` ou compensation inconnue, ne pas reessayer la mutation, marquer/quarantainer l'intention si SQLite reste disponible, puis imposer probe/delete explicite du namespace. Au restart, reprendre uniquement les intentions C1 non terminales du namespace de campagne et converger vers `Delete -> VerifyEmpty`; ne jamais provisionner pendant le cleanup.

Le journal d'intention est une table de spike jetable, pas un schema produit. Il ne donne jamais le droit de reutiliser le harnais, son API ou ses dependances. Les scenarios couvrent create, read medie, replace, delete, restart, refus, verrou, crash avant/apres chaque effet, logout/revocation, doublons, compensation et cleanup du namespace au demarrage/teardown. `Delete` reste idempotent sur succes, erreur, cancel et teardown; `VerifyEmpty` ne lit que handles/metadonnees.
...
| STO-VLT-01 | `VaultHarness` natif jetable | create/read medie/replace/delete/restart/refus/verrou/crash/logout, doublons et cleanup | ordre et etats exacts; `MutationUnknown` fail-closed; zero C4 hors coffre; handle seul en DB; namespace vide |
...
cargo run --locked --release -p fluent-storage-spike -- vault-harness --backend auto --cases create,read-mediated,replace,delete,restart,denied,locked,crash,logout,cleanup --fault-matrix all --output $env:FLUENT_ARTIFACT_ROOT
...
- zero C4 est trouve hors du coffre par le scan interne; aucune valeur ni empreinte C4 ne sort du harnais, et la base ne contient qu'intention C1 et handle opaque;
- chaque mutation est idempotente et classee `Applied`, `NotApplied` ou `MutationUnknown`; tout outcome inconnu est fail-closed, compense/quarantine puis nettoye explicitement sans retry automatique;
...
Tout C4 hors coffre, empreinte C4 dans un artefact, namespace non vide apres cleanup, retry apres `MutationUnknown`, canari de retention conserve dans une copie possedee, base hybride/corrompue apres scenario supporte, audio persiste, egress ou perte silencieuse est `FAIL`. Un coffre ou banc OS absent est `unavailable_environment` et empeche un `PASS` pour cette plateforme.
...
## 6. Reevaluation F-01/F-02 et gate F-03
...
### 6.3 F-03 — precondition bloquante avant manifests/lockfiles

F-03 s'execute **avant** toute mutation d'un `Cargo.toml`, `Cargo.lock`, manifest de package, action CI ou source de dependance, y compris pour un prototype jetable ou `VaultHarness`. Le manager attribue d'abord l'ownership du fichier partage. La revue versionnee compare la baseline et la proposition sans committer la mutation et couvre:

- inventaire complet du lockfile avec paquets directs/transitifs ajoutes, retires ou modifies, features et chemins inverses pertinents;
- advisories RustSec/OSV et statut yanked pour chaque version candidate;
- licences SPDX, obligations et compatibilite, sans deduire la licence d'un nom de depot;
- sources registre/Git/path/tarball, checksums/integrites et toute source nouvelle;
- diff exact des manifests/lockfiles et raison de chaque changement;
- outil, version, hash du binaire et version/hash de la base d'advisories; si aucun outil approuve n'est disponible, revue manuelle documentee avec les memes champs et sources primaires archivees;
- pour chaque exception: identifiant exact, portee/lockfile hash, justification, owner, date d'expiration, condition de sortie et verdict explicite.

Une exception expiree, sans owner ou sans condition de sortie bloque le commit. L'indisponibilite d'un scanner n'autorise ni une sortie vide ni une version flottante: la revue manuelle versionnee devient obligatoire ou le lot est `BLOCKED`.

Artefacts F-03 minimaux: `dependency-inventory.json`, `dependency-diff.json`, `advisories.json`, `licenses.json`, `sources.json`, `exceptions.json`, `tooling.json` et un verdict signe `PASS`/`FAIL` par l'owner et security. Ils ne contiennent ni token de registre, ni variable d'environnement, ni chemin utilisateur.

Le futur controle CI bloquant inclut une fixture negative isolee, sans modifier le manifest produit: elle introduit tour a tour une advisory/yanked, une licence refusee, une source Git/path non allowlistee et une exception expiree, puis prouve que chaque cas fait echouer le job. La fixture et l'automatisation sont planifiees avant Gate 12; ce plan ne les implemente pas. SBOM et notices tierces restent egalement dus au Gate 12 et avant toute distribution, sans etre revendiques par le spike.

Verdict F-03: `PASS` seulement si l'inventaire et le diff sont complets, les outils/revues sont reproductibles, aucune exception invalide n'existe et security autorise la mutation precise. Toute mutation realisee avant ce verdict, source inconnue, paquet yanked/advisory exploitable sans disposition ou licence non tranchee est `FAIL` et doit etre retiree/rollbackee avant reprise.

### 6.4 Commandes de collecte F-01/F-02/F-03
...
Si `cargo audit`/`cargo deny` ne sont pas encore verrouilles et approuves, le responsable supply chain les provisionne avant execution; lancer une version flottante ne donne pas une preuve qualifiante. Le rapport consigne `--version`, provenance et hash des executables et de la base advisory. Les sorties alimentent les artefacts F-03 et un rapport de reachability redige, jamais une pretention binaire fondee sur le seul `cargo tree`.
...
2. Attribuer ownership des nouveaux fichiers, dependances et lockfile; preparer toute resolution candidate hors des manifests suivis, obtenir le verdict F-03 `PASS`, puis seulement autoriser la mutation exacte revue.
...
5. Construire le harnais stockage avec fixtures jetables, puis migrations/crash/retention avant `VaultHarness` sur un coffre OS reel de test.
...
- matrices audio/stockage/F-01/F-02/F-03 avec `PASS`, `FAIL`, `not_run`, `unavailable_environment` ou `OPEN_UNPROVEN` justifies;
...
- preuve `VaultHarness` par OS execute, avec etats/compensation/namespace vide et sans valeur ni empreinte C4 archivee;
...
Les prototypes utilisent uniquement des fixtures jetables et des namespaces `VaultHarness` propres au `run-id`. Le rollback supprime les binaires/fixtures/types/dependances temporaires du harnais, execute `Delete -> VerifyEmpty` sur ses namespaces et revient aux documents/contrats acceptes; aucune donnee utilisateur ou migration de production n'existe. Une dependance ou permission introduite pour le spike n'est promue qu'apres ADR, gate F-03 et campagne `PASS`; le harnais C4 n'est jamais promu. Un echec audio conserve le core et remplace l'adaptateur/file derriere le contrat; un echec SQLite/coffre conserve le modele logique et remplace le harnais/adaptateur apres export logique de fixture C1-C2 seulement.
