...
Statut: **baseline Phase 00 alignee sur les decisions confirmees le 2026-08-09**, a revalider aux phases 02, 10, 11, 12 et 13.
...
Zero-history est le defaut; l'historique texte local n'existe qu'apres opt-in avec retention configurable et aucun audio n'est persiste par defaut. Le chemin local ne requiert aucun compte et reste utilisable sans Cloud apres installation volontaire des modeles requis. Le Cloud est absent du MVP; son architecture future reste facultative. L'activation post-MVP d'un service Cloud, de la synchronisation ou de la telemetrie ne vaut jamais consentement pour une autre finalite.

## Portee et limites

La portee couvre l'application desktop Tauri, le coeur Rust, les adaptateurs macOS/Linux/Windows, les modeles locaux, le stockage local et l'updater du MVP, ainsi que les frontieres Cloud post-MVP facultatives. Elle couvre le microphone, les transcriptions, l'historique texte local opt-in, le dictionnaire, le contexte obtenu par accessibilite, l'OCR, le presse-papiers, les identifiants de fenetre/application, les diagnostics, les credentials et la supply chain.
...
9. Disponibilite du chemin local sans compte ni Cloud et exactitude du texte injecte dans la cible.
...
|---|---|---|---|---|---|
| TM-01 | Critique | Un futur mode ASR ou de reecriture Cloud envoie audio, texte ou contexte apres une activation ambigue, globale ou heritee. | D-10 exclut le Cloud du MVP et ne conserve que des ports futurs; `R-006` identifie la fuite audio/contexte. Test reseau du MVP puis de toute future fonction avec tous les opt-in desactives. | Conversation ou contenu d'application transmis a l'insu de l'utilisateur. | Aucun endpoint Cloud dans le MVP; apres le MVP, consentement avant transfert distinct par finalite et type de donnee, recapitulatif destination/retention, revocation immediate et test d'absence d'egress. |
...
| TM-10 | Critique | Une dependance npm/cargo/python, action CI ou runner compromet les artefacts. | `R-011`, phases 01 et 12 demandent lockfiles et audits. D-11 maintient le depot public, D-12 adopte Apache-2.0 et D-13 reserve une monetisation Cloud future; verifier SBOM, provenance, licences propres aux modeles et reproductibilite. | Compromission globale des clients, divulgation de secret du depot public ou distribution illicite d'un modele. | Lockfiles, versions d'actions par digest, moindre privilege CI, builds reproductibles, SBOM, scans secrets/licence/vulnerabilite, provenance signee, licences des modeles distinctes, revue des mainteneurs/dependances et secrets de release isoles; aucune exception pour un composant gratuit ou payant. |
...
| TM-16 | Haute | Un moteur local ou futur service Cloud indisponible pousse silencieusement vers un autre fournisseur ou fait perdre le texte brut. | D-09 impose un chemin local sans compte et D-10 exclut le Cloud du MVP; la Phase 09 exige un fallback brut exact. | Transfert non consenti, indisponibilite ou alteration du contenu. | Aucun failover vers le Cloud ou un autre fournisseur sans consentement correspondant; fallback deterministe au texte brut; file locale chiffree seulement si l'utilisateur choisit la reprise; erreurs explicites. Une panne Cloud post-MVP ne doit jamais rendre le chemin local inutilisable. |
...
- Cloud post-MVP indisponible ou opt-in revoque: ne pas basculer vers un autre service et ne pas mettre en file audio/contexte pour envoi ulterieur; conserver le chemin local utilisable.
